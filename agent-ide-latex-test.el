...
(ert-deftest agent-ide-latex-input-edits-rerender-without-changing-text-or-face ()
  (agent-ide-latex-test--isolated
    (let ((settings '(dvisvgm 1.0 "#000000"))
          (image-x '(image :type svg :data "x"))
          (image-y '(image :type svg :data "y")))
      (dolist (entry (list (cons "$x$" image-x) (cons "$y$" image-y)))
        (puthash (agent-ide-latex--cache-key (car entry) settings)
                 (list :status 'done :image (cdr entry)) agent-ide-latex--cache))
      (cl-letf (((symbol-function 'agent-ide-latex--settings) (lambda () settings)))
        (with-temp-buffer
          (insert (propertize "$x$" 'face 'bold))
          (buffer-enable-undo)
          (set-buffer-modified-p nil)
          (agent-ide-latex-update-input 1 (point-max))
          (should (eq (get-text-property 1 'display) image-x))
          (should-not (buffer-modified-p))
          (should-not buffer-undo-list)
          (should-not (text-property-not-all 1 (point-max) 'face 'bold))
          (goto-char 2)
          (agent-ide-latex-update-input 1 (point-max))
          (should-not (get-text-property 1 'display))
          (delete-char 1)
          ...
          (goto-char (point-max))
          (agent-ide-latex-update-input 1 (point-max))
          (should (eq (get-text-property 1 'display) image-y))
          (should (equal (buffer-substring-no-properties 1 (point-max)) "$y$"))
          (should-not (text-property-not-all 1 (point-max) 'face 'bold))
          ;; Removing the delimiter must invalidate even a previously cached image.
          (delete-char -1)
          (agent-ide-latex-update-input 1 (point-max))
          (should-not (text-property-not-all 1 (point-max) 'display nil))
          (should-not (get-text-property 1 'agent-ide-latex-key))
...
(ert-deftest agent-ide-latex-input-retains-pending-workers-and-cleans-inherited-tail ()
  (agent-ide-latex-test--isolated
    (cl-letf (((symbol-function 'agent-ide-latex--settings)
               (lambda () '(dvisvgm 1.0 "#000000")))
              ((symbol-function 'agent-ide-latex--start-next) #'ignore))
      (with-temp-buffer
        ...
        (let* ((key (car agent-ide-latex--queue))
          ...
          (should-not (get-text-property 4 'agent-ide-latex-key))
          (agent-ide-latex--apply (car (plist-get record :waiters))
                                '(image :type svg :data "x") nil)
          (should (get-text-property 1 'display))
          (should-not (get-text-property 4 'display)))))))

(ert-deftest agent-ide-latex-input-rejects-stale-worker-after-edit ()
  (agent-ide-latex-test--isolated
    (cl-letf (((symbol-function 'agent-ide-latex--settings)
               (lambda () '(dvisvgm 1.0 "#000000")))
              ((symbol-function 'agent-ide-latex--start-next) #'ignore))
      (with-temp-buffer
        (insert "$x$")
        (agent-ide-latex-update-input 1 (point-max))
        (let ((waiter (car (plist-get (gethash (car agent-ide-latex--queue)
                                             agent-ide-latex--cache) :waiters))))
          (goto-char 2)
          (delete-char 1)
          (insert "y")
          (agent-ide-latex-update-input 1 (point-max))
          (agent-ide-latex--apply waiter '(image :type svg :data "stale") nil)
          (should-not (text-property-not-all 1 (point-max) 'display nil))
          (should (= (length agent-ide-latex--queue) 2)))))))

(ert-deftest agent-ide-latex-input-skips-code-and-can-disable-previews ()
  (agent-ide-latex-test--isolated
    (cl-letf (((symbol-function 'agent-ide-latex--settings)
               (lambda () (when agent-ide-latex-preview '(dvisvgm 1.0 "#000000"))))
              ((symbol-function 'agent-ide-latex--start-next) #'ignore))
      (with-temp-buffer
        (insert "`$code$`\n```latex\n$fenced$\n```\n\\[x\n+y\\]")
        (agent-ide-latex-update-input 1 (point-max))
        (should (= (length agent-ide-latex--queue) 1))
        (should (equal (plist-get (gethash (car agent-ide-latex--queue)
                                         agent-ide-latex--cache) :source) "\\[x\n+y\\]"))
        (let ((agent-ide-latex-preview nil))
          (agent-ide-latex-update-input 1 (point-max)))
        (should-not agent-ide-latex--input-fragments)
        (should-not (text-property-not-all 1 (point-max) 'agent-ide-latex-key nil))))))
