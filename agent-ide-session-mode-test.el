...
(ert-deftest agent-ide-session-mode-previews-editable-formulas ()
  "Prompt previews preserve editable text, styling and the submitted source."
  (let* ((agent-ide-latex--cache (make-hash-table :test 'equal))
         (settings '(dvisvgm 1.0 "#000000"))
         (image '(image :type svg :data "input")))
      ...
      (with-temp-buffer
        ...
        (let ((session (agent-ide-session-mode-test--session)))
          ...
          (let ((start (agent-ide-session-input-start-marker session)))
            (should (eq (get-text-property start 'display) image))
            (should (eq (get-text-property start 'face) 'agent-ide-user-prompt-face))
            (should-not (get-text-property start 'read-only))
            (should (equal (agent-ide-renderer-current-input session) "$x$"))
            (goto-char (1+ start))
            (run-hooks 'post-command-hook)
            (should-not (get-text-property start 'display))
            (insert "y")
            (delete-char -1)
            (goto-char (agent-ide-session-input-end-marker session))
            (run-hooks 'post-command-hook)
            (should (eq (get-text-property start 'display) image)))
          ;; A new input must not reuse the preceding prompt's tracked ranges.
          (agent-ide-renderer-freeze-current-input session)
          (agent-ide-renderer-create-prompt session)
          (insert "$x$")
          (run-hooks 'post-command-hook)
          (should (eq (get-text-property (agent-ide-session-input-start-marker session)
                                         'display) image))
          (agent-ide-renderer-replace-current-input session "$x$")
          (run-hooks 'post-command-hook)
          (should (eq (get-text-property (agent-ide-session-input-start-marker session)
                                         'display) image))
          (should (equal (agent-ide-renderer-current-input session) "$x$")))))))
